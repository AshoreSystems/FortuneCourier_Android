...
public class PaymentAssociateActivity extends Activity implements View.OnClickListener{
    ...
    private void init(){
        ...
        stripe = new Stripe(PaymentAssociateActivity.this,getResources().getString(R.string.publishable_key));
        ...
        if (cd.isConnectingToInternet()){
            ...
            CustomDialogForHelp customDialogForHelp = new CustomDialogForHelp(PaymentAssociateActivity.this,getResources().getString(R.string.app_name),getResources().getString(R.string.err_msg_internet));
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.btn_pay:
                ...
                if(strCustomerId.equalsIgnoreCase("")){
                    if(card_multiline_widget.getCard()==null){
                        CustomDialogForHelp customDialogForHelp = new CustomDialogForHelp(PaymentAssociateActivity.this,getResources().getString(R.string.app_name),getResources().getString(R.string.err_msg_payment));
                        customDialogForHelp.show();
                    }else {
                        getToken();
                    }
                   ...
                   /* if (!editText_cvv.getText().toString().isEmpty()) {
                       if (checkbox_save_card.isChecked()) {
                         startActivity(new Intent(PaymentAssociateActivity.this, VerifyPincodeCustomerActivity.class).putExtra("token", "").putExtra("customer_id", strCustomerId).putExtra("save_card_details", "1").putExtra("card_no_last_digits", "0").putExtra("cvv",editText_cvv.getText().toString().trim()));
                         finish();
                       } else {
                         startActivity(new Intent(PaymentAssociateActivity.this, VerifyPincodeCustomerActivity.class).putExtra("token", "").putExtra("customer_id", strCustomerId).putExtra("save_card_details", "0").putExtra("card_no_last_digits", strLastDigits).putExtra("cvv",editText_cvv.getText().toString().trim()));
                         finish();
                       }
                      }else {
                        CustomDialogForHelp customDialogForHelp = new CustomDialogForHelp(PaymentAssociateActivity.this,getResources().getString(R.string.app_name),getResources().getString(R.string.err_msg_enter_cvv));
                        customDialogForHelp.show();
                    }*/

                    // startActivity(new Intent(PaymentAssociateActivity.this,VerifyPincodeCustomerActivity.class).putExtra("token","").putExtra("customer_id",strCustomerId).putExtra("save_card_details","0").putExtra("card_no_last_digits","0"));
                   ...
                   /* if (cd.isConnectingToInternet()) {
                        ...
                        if (PaymentAssociateActivity.this != null) {
                            CustomDialogForHelp customDialogForHelp = new CustomDialogForHelp(PaymentAssociateActivity.this,getResources().getString(R.string.app_name),getResources().getString(R.string.err_msg_internet));
                            customDialogForHelp.show();
            ...
            case R.id.img_info:
                hideKeyboard();
                DBInfo dbInfo = new DBInfo(PaymentAssociateActivity.this);
                CustomDialogForHelp customDialogForHelp = new CustomDialogForHelp(PaymentAssociateActivity.this,"Help",dbInfo.getDescription("Payment Screen"));
                customDialogForHelp.show();
                break;
    ...
    private void verifyCard(final String strCvv) {
        ...
        String URL = getResources().getString(R.string.url_domain_associate) + getResources().getString(R.string.url_verify_card);
        ...
        VolleyUtils.POST_METHOD(PaymentAssociateActivity.this, URL, params, new VolleyResponseListener() {
            @Override
            ...
            public void onResponse(Object response) {
                ...
                try {
                    ...
                    if(Json_response.getString(JSONConstant.STATUS).equalsIgnoreCase(JSONConstant.SUCCESS)) {
                        if (checkbox_save_card.isChecked()) {
                            startActivity(new Intent(PaymentAssociateActivity.this, VerifyPincodeAssociateActivity.class).putExtra("token", "").putExtra("customer_id", strCustomerId).putExtra("save_card_details", "1").putExtra("card_no_last_digits", strLastDigits).putExtra("cvv",strCvv));
                            finish();
                        } else {
                            startActivity(new Intent(PaymentAssociateActivity.this, VerifyPincodeAssociateActivity.class).putExtra("token", "").putExtra("customer_id", strCustomerId).putExtra("save_card_details", "0").putExtra("card_no_last_digits","0" ).putExtra("cvv",strCvv));
                            finish();
                        }
                    }else {
                        CustomDialogForHelp customDialogForHelp = new CustomDialogForHelp(PaymentAssociateActivity.this,getResources().getString(R.string.app_name),Json_response.getString(JSONConstant.MESSAGE));
                        customDialogForHelp.show();
                    }
                    closeProgressDialog();
                ...
                if (!card.getNumber().isEmpty()) {
                    ...
                    stripe.createToken(card, getResources().getString(R.string.publishable_key), new TokenCallback() {
                        ...
                        public void onSuccess(Token token) {
                            ...
                            if(checkbox_save_card.isChecked()){
                                startActivity(new Intent(PaymentAssociateActivity.this,VerifyPincodeAssociateActivity.class).putExtra("token",token.getId()).putExtra("customer_id","").putExtra("save_card_details","1").putExtra("card_no_last_digits",strLastDigits).putExtra("cvv",strCVV));

                                finish();

                            }else {
                                startActivity(new Intent(PaymentAssociateActivity.this,VerifyPincodeAssociateActivity.class).putExtra("token",token.getId()).putExtra("customer_id","").putExtra("save_card_details","0").putExtra("card_no_last_digits","0").putExtra("cvv",strCVV));
    ...
    private void showProgressDialog(){
        pDialog = new ProgressDialog(PaymentAssociateActivity.this);
    ...
    private void sendToken(String strToken){
        String URL = getResources().getString(R.string.url_domain_associate) + getResources().getString(R.string.url_pay);
        Map<String, String> params = new HashMap<String, String>();
        params.put(JSONConstant.A_ID,mSessionManager.getStringData(SessionManager.KEY_A_ID));
                ...
                try {
                    ...
                    if(Json_response.getString(JSONConstant.STATUS).equalsIgnoreCase(JSONConstant.SUCCESS)) {
                        if(checkbox_save_card.isChecked()) {
                            ...
                            startActivity(new Intent(PaymentAssociateActivity.this, LabelAssociateActivity.class).putExtra(JSONConstant.STATUS, JSONConstant.SUCCESS).putExtra(JSONConstant.MESSAGE, Json_response.getString(JSONConstant.MESSAGE)).putExtra("pdf", Json_response.getString("pdf_label")).putExtra(JSONConstant.MASTER_TRACKING_NO, Json_response.getString(JSONConstant.MASTER_TRACKING_NO)).putExtra("save_card_details", "1").putExtra("card_no_last_digits", "0"));
                            finish();
                        }else{
                            startActivity(new Intent(PaymentAssociateActivity.this,LabelAssociateActivity.class).putExtra(JSONConstant.STATUS,JSONConstant.SUCCESS).putExtra(JSONConstant.MESSAGE,Json_response.getString(JSONConstant.MESSAGE)).putExtra("pdf",Json_response.getString("pdf_label")).putExtra(JSONConstant.MASTER_TRACKING_NO,Json_response.getString(JSONConstant.MASTER_TRACKING_NO)).putExtra("save_card_details","0").putExtra("card_no_last_digits",strLastDigits));
                            finish();
                        ...
                        //startActivity(new Intent(PaymentAssociateActivity.this,));
                       /* CustomDialogForHelp customDialogForHelp = new CustomDialogForHelp(PaymentAssociateActivity.this,getResources().getString(R.string.app_name),Json_response.getString(JSONConstant.MESSAGE));
                        customDialogForHelp.show();*/
                    }else {
                        JSONObject jsonObject = Json_response.getJSONObject(JSONConstant.ERROR_MESSAGES);
                        startActivity(new Intent(PaymentAssociateActivity.this,LabelAssociateActivity.class).putExtra(JSONConstant.STATUS,"Failure").putExtra(JSONConstant.MESSAGE,jsonObject.has(JSONConstant.MESSAGE)));
                        finish();
                        if (jsonObject.has(JSONConstant.MESSAGE)) {
                          /*  CustomDialogForHelp customDialogForHelp = new CustomDialogForHelp(PaymentAssociateActivity.this,getResources().getString(R.string.app_name),jsonObject.getString(JSONConstant.MESSAGE));
                            customDialogForHelp.show();*/
    ...
    private void applyCoupon(final String strCode){
        showProgressDialog();
        String URL = getResources().getString(R.string.url_domain_associate) + getResources().getString(R.string.url_apply_coupon_code);
        Map<String, String> params = new HashMap<String, String>();
        params.put(JSONConstant.A_ID,mSessionManager.getStringData(SessionManager.KEY_A_ID));
        params.put(JSONConstant.SHIPMENT_ID,mSessionManager.getStringData(SessionManager.KEY_SHIPMENT_ID));
        //params.put("currency",mSessionManager.getStringData(SessionManager.KEY_CURRENCY_UNIT));
    ...
    private void getSavedCards(){
        String URL = getResources().getString(R.string.url_domain_associate) + getResources().getString(R.string.url_get_saved_cards);
        Map<String, String> params = new HashMap<String, String>();
        params.put(JSONConstant.A_ID,mSessionManager.getStringData(SessionManager.KEY_A_ID));
                    ...
                    if(Json_response.getString(JSONConstant.STATUS).equalsIgnoreCase(JSONConstant.SUCCESS)) {
                        ...
                        if(jsonArray.length()>0){
                            ...
                            for (int i = 0;i<jsonArray.length();i++){
                                ...
                                RadioButton radiobutton = new RadioButton(PaymentAssociateActivity.this);
