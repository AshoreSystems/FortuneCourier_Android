...
public class ChangePincodeAssociateActivity extends Activity implements View.OnClickListener {
    ...
    public void onClick(View v) {
        switch (v.getId()){
            ...
            case R.id.tv_forgot_pincode:
                hideKeyboard();
                startActivity(new Intent(ChangePincodeAssociateActivity.this,ForgotPincodeAssociateActivity.class));
                break;
        }
    }
    ...
    private void validate(){
        if(cd.isConnectingToInternet()) {
            if (pinView_old_pincode.getText().toString().isEmpty() || pinView_old_pincode.getText().toString().length() < 6) {
                CustomDialogForHelp customDialogForHelp = new CustomDialogForHelp(ChangePincodeAssociateActivity.this, getResources().getString(R.string.app_name), getResources().getString(R.string.err_msg_pincode));
                ...
                CustomDialogForHelp customDialogForHelp = new CustomDialogForHelp(ChangePincodeAssociateActivity.this, getResources().getString(R.string.app_name), getResources().getString(R.string.err_msg_pincode_not_matched));
                customDialogForHelp.show();
            } else {
                changePincode();
            }
        }else {
            CustomDialogForHelp customDialogForHelp = new CustomDialogForHelp(ChangePincodeAssociateActivity.this, getResources().getString(R.string.app_name), getResources().getString(R.string.err_msg_internet));
            customDialogForHelp.show();
        }
    }

    private void changePincode(){
        progressBar = new ProgressDialog(ChangePincodeAssociateActivity.this);
        ...
        String URL = getResources().getString(R.string.url_domain_associate) + getResources().getString(R.string.url_change_pin);
        Map<String, String> params = new HashMap<String, String>();
        params.put(JSONConstant.A_ID,mSessionManager.getStringData(SessionManager.KEY_A_ID));
        params.put("a_old_pin",pinView_old_pincode.getText().toString().trim());
        params.put("a_security_pin",pinView_new_pincode.getText().toString().trim());
        params.put("a_confirm_security_pin",pinView_re_enter_pincode.getText().toString().trim());

        System.out.println("=>"+params.toString());

        VolleyUtils.POST_METHOD(ChangePincodeAssociateActivity.this, URL, params, new VolleyResponseListener() {
            @Override
            ...
            public void onResponse(Object response) {
                ...
                try {
                    ...
                    if(Json_response.getString(JSONConstant.STATUS).equalsIgnoreCase(JSONConstant.SUCCESS)) {
                        Toast.makeText(ChangePincodeAssociateActivity.this,Json_response.getString(JSONConstant.MESSAGE),Toast.LENGTH_SHORT).show();
                        finish();
                    }else {
                        JSONObject jsonObject = Json_response.getJSONObject(JSONConstant.ERROR_MESSAGES);

                        if (jsonObject.has(JSONConstant.MESSAGE)) {
                            CustomDialogForHelp customDialogForHelp = new CustomDialogForHelp(ChangePincodeAssociateActivity.this,getResources().getString(R.string.app_name),jsonObject.getString(JSONConstant.MESSAGE));
                            customDialogForHelp.show();
                        }
                    }
                    progressBar.dismiss();
